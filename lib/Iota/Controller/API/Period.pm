...
package Iota::Controller::API::Period;
use utf8;
use Moose;
...
sub week_GET {
    ...
    $self->status_ok(
        ...
        entity => {
            options => [
                map { +{ value => $_->{period_begin}, text => $_->{period_begin} . " - Semana $_->{week_num}" } }
                  @$list
            ]
        }
    );
...
sub year_GET {
    ...
    $self->status_ok( $c,
        entity => { options => [ map { +{ value => $_ . '-01-01', text => "$_" } } ( 2000 .. $today ) ] } );
}
...
sub month_GET {
    ...
    $self->status_ok(
        ...
        entity => {
            options => [
                map { +{ value => $year . '-' . sprintf( '%02s', $_ ) . '-01', text => "$year - " . $meses[ $_ - 1 ] } }
                  ( 1 .. $max )
            ]
        }
    );
